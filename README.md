...
自动识别 OpenList / AList 站点的同目录弹幕文件并自动载入；其它网页播放器走通用适配——控件栏注入开关 + 设置菜单 + 手动载入。任意原生 `<video>` 或未知播放器也能用（探测到常见播放器控件栏则注入其中，否则退回右下浮层按钮）。支持弹弹play 在线搜索/智能匹配弹幕（含 AI 增强 + 亮暗主题）。
...
3. **（仅自动载入需要）准备弹幕文件**：见下方「OpenList 自动载入」。
4. 打开任意视频页面，脚本自动生效。OpenList 默认端口 `5244` 的页面会自动识别并启用同目录自动载入；其它页面在视频出现后挂载弹幕（无自动载入，用菜单手动载入）。
...
| | |
|:---:|:---:|
| <img src="https://www.retr0.xyz/blog-img/20260626123308168.png" width="100%" alt="OpenList + ArtPlayer"><br><b>OpenList 使用展示</b> | <img src="https://www.retr0.xyz/blog-img/1782447143356.png" width="100%" alt="DPlayer"><br><b>DPlayer 使用展示</b> |
...
| **ArtPlayerAdapter** | 任意站点的 ArtPlayer（`.art-video-player`） | `.art-video-player` | 同上 | ❌（手动载入） |
| **DPlayerAdapter** | DPlayer（`.dplayer`） | `.dplayer-video-wrap` | 注入 `.dplayer-icons-right`（原生 `.dplayer-icon` 样式，追加末尾） | ❌（手动载入） |
| **GenericAdapter** | 原生 `<video>` / 未知播放器 | `video.parentElement` | 探测控件栏命中则注入其中，否则右下浮层按钮 | ❌（手动载入） |

> DPlayer 现在有**专用适配器**（原生的 `.dplayer-icon` 按钮注入到右侧图标组，外观与自带控件一致），不再走 GenericAdapter。

GenericAdapter 的控件栏探测会向上最多 6 层查找常见播放器的控件栏：

> Video.js（`.vjs-control-bar`）· Plyr（`.plyr__controls`）· MediaElement（`.mejs-controls`）· JW Player（`.jw-controlbar`）· Clappr · Shaka · vue-video-player · 通用 `[class*="control-bar"]` 等

命中则按钮以中性样式 `__titan_dm_ctrlbtn__` 注入该控件栏（图标色继承控件栏），探测不到才退回右下圆形浮层按钮。

> **如果你的播放器按钮没进控件栏（退回了浮层）**：告诉我它的控件栏 class，加进脚本里的 `GENERIC_CTRL_SELECTORS` 列表即可。ArtPlayer / DPlayer / OpenList 这三种已走专用适配器，无需手动加。
...
> 默认值指向作者部署的 Worker，开箱即用。重度用户建议按 [worker/README.md](./worker/README.md) 自部署以避免流量集中触发配额。
...
| 设置项 | 说明 |
|---|---|
| 启用 AI 开关 | **开启后「✨ 智能匹配」按钮才显示**，且使用 AI 提取文件名处理番剧名和集数 |
| API 地址 | OpenAI 兼容的 base URL，如 `https://api.deepseek.com/v1` |
| Key | 你的 API key（`sk-...`） |
| 模型 | 如 `deepseek-v4-flash` |

> 开关开启 → 智能匹配必用 AI（LLM 提取番剧名+集号再搜索，成功率远高于直接文件名匹配）。开关关 → 按钮隐藏。配置不全时点匹配会提示引导去填写，不静默回退。Key 存本地 localStorage，调用经 Worker `/llm` 转发（解决浏览器 CORS），不在 Worker 存储。
...
脚本**内置默认 Worker**（`ddplay.retr0.xyz`），安装后开箱即用，无需配置：

1. 菜单 →「🌐 弹弹play 搜索弹幕」→ 输入番剧名（中/日/罗马音/英文，≥2 字）→ 选作品 → 选剧集 → 自动拉取并载入弹幕。
2. 或「✨ 智能匹配当前视频」：脚本取视频文件名（去扩展名）+ 大小，调弹弹play `/match` 接口；精确命中单集会弹窗确认载入，多候选则列出供选，无结果则回退用文件名搜索。

> 载入后会保留视频当前位置（不重置进度），并整合第三方弹幕源（A/B/C 站，`withRelated=true`）。

> **想换自己的 Worker？** 菜单 →「⚙ 通用设置」→「弹弹play 代理」分区填入自己的 Worker URL + token，留空即用默认。建议重度用户自部署，避免流量集中到默认 Worker 触发配额（见下）。

### AI 智能匹配增强

视频文件名常含字幕组/分辨率/编码等杂质（如 `[ANi]_孤独摇滚！_01_[1080p][BDRip][x265][FLAC]`），直接拿去匹配成功率有限。启用 AI 后，脚本会先用 LLM 从文件名提取干净的**番剧名 + 集号**，再用提取结果精确搜索，成功率大幅提升。

需要两步：① 在通用设置的「AI 配置」里**打开「启用 AI」开关**并填写 API 地址/Key/模型；② 保存后「✨ 智能匹配」按钮才会出现在菜单和搜索弹窗里。
...
弹弹play 开放平台 API 需要 AppId + AppSecret 鉴权。**密钥不能硬编码进前端脚本**（任何人都能从脚本里扒走）。因此所有对 `api.dandanplay.net` 以及 LLM API 的请求先经 Cloudflare Worker 代理：密钥只存在于 Worker 的环境变量（Secrets）里，脚本侧只持有「Worker URL + 防滥用 token」。

脚本内置了默认 Worker（`ddplay.retr0.xyz`）及其 token，开箱即用。这两个值不是密钥，但若脚本被大量分发所有用户流量会集中打到默认 Worker，建议重度用户自部署。
